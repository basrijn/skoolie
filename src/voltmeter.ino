...
#define NUM_SAMPLES 10    // We will sample the voltage 10 times
...
float upperBatteryVoltage = 12.7;
float lowerBatteryVoltage = 10.5;

unsigned long minuteAgoLength = 10* 1000; // Set the length of how far we want to look back for the direction of change
...
void setup() {
  ...
  // We need an accurate 5V reference tied to the ARf pin
...
void loop() {
  ...
  // Do a voltage read and update maximum and minimum
  ...
  if ( (millis() - minuteAgoTS) >  minuteAgoLength) {
    ...
    // If the voltage hasn't changed, don't update anything. Battery charging might be slow
  ...
  // Draw th triangles to show charge direction. If 0, then nothing draws
...
float readVoltage() {
  ...
  Serial.println( ( (float)sum / (float)NUM_SAMPLES ) / 1023.0);
  Serial.println((( (float)sum / (float)NUM_SAMPLES ) / 1023.0) * 5.0);
  Serial.println((( (float)sum / (float)NUM_SAMPLES ) / 1023.0) * 5.0 * 12.0);
  ...
  float volt = ( ( (float)sum / (float)NUM_SAMPLES ) / (float)1023.0) * (float)5.0 * (float)3.2 ; // 2.2k + 1k voltage divider and a 5V reference (for 16V)
...
void neoByVoltage(float curVoltage) {
  ...
  if ( (minuteAgoDirection > 0) && (curVoltage > upperBatteryVoltage) ) {
    // Do some cool fading
    Serial.println(".. We seem to be charging");
    neoFader();
  }
